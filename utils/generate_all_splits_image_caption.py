...
def construct_prompt(city_name: str, page_num: str = "") -> str :
    prompt = f"""你是城市信息总结专家，当前需要你分析的城市是{city_name}。请根据图中显示的内容进行描述，提取并总结该页的核心信息，请确保：
                1.陈述图片中看到的内容，不添加任何超出该图片内容的背景、原因或推断；
                2.保持描述的客观性和准确性，避免生成与图片无关的内容。
                请用中文回答，语言简洁、逻辑清晰，总结全文的字数控制在 100-120 字之间。如需引用图中文字，可用“”括起。
                """
...
def main():
    ...
    parser.add_argument('--image_root_dir', type=str, 
                      default="/home/linux/yyj/colpali/finetune/pdf2images_all_splits",
                      help='Image directory path')
    parser.add_argument('--output_file', type=str, 
                      default='/home/linux/yyj/colpali/finetune/mmlm-rag/utils/all_splits_image_cap_pairs.json',
